...
for line in readfile(filename1):
    ...
    temp += 1
while True :
    x = 8#input("Which system do you want to visualize ?")
    try :
        if 1 <= int(x) <= temp:
            break
...
def Compute_force_G_on_a(gravconst_mata,All_r_interaction_lisa,All_modr3a, indexplanet):
    ...
    for i in range(a-1):
        if i == indexplanet:
            continue
        fx_a_x.append(gravconst_mata[i]*All_r_interaction_lisa[i][0]/All_modr3a[i])   #à faire en c
        fy_a_x.append(gravconst_mata[i]*All_r_interaction_lisa[i][1]/All_modr3a[i])
        fz_a_x.append(gravconst_mata[i]*All_r_interaction_lisa[i][2]/All_modr3a[i])
    return fx_a_x, fy_a_x,fz_a_x
...
while t<tsimu*365*daysec:
    ...
    for i in range(a-1):
        fx, fy, fz = Compute_force_G_on_a(Gravconst_mat[i],All_r_interaction_list[i],All_modr3[i], i)
